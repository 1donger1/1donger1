...
{
    // 申请堆上空间 size*数据类型的大小
    array->dp = (ElementType *)malloc(size * sizeof(ElementType)); //(强制类型转换)malloc(sizeof(int)*个数)
    array->len = 0;
    array->size = size;
    // 判断是否申请成功
    if (array->dp == NULL)
...
{
    // printf("Use ReallocArray!\n");
    // 保存原先内存的备份指针
    // ElementType *temp = array->dp;
    // // 给dp指针申请一块新内存，大小为数据类型*原先的空间个数*2
    // array->dp = (ElementType *)malloc(sizeof(ElementType) * array->size * GROWTH_FACTOR);
    // // 判断是否扩容成功
    // if (array->dp == NULL)
    // {
    //     // 申请失败，打印错误信息，返回FALSE
    ...
    // // 空间扩展到原先两倍
    // array->size = array->size * GROWTH_FACTOR;
