...
/*******************************************************************/
void SaveGameProgress(struct Player *player, DMArray *monsters, const char *fileName)
{
    ...
    for (int i = 0; i < monsters->len; i++)
    {
        ...
        if (monster->isAlive) // Check if monster is alive before saving
        {
            fprintf(file, "%s %d %d %d %d %d\n", monster->name.string, monster->x + 1, monster->y + 1, monster->HP, monster->attack, monster->isAlive);
        }
    }
...
int LoadGameProgress(struct Player *player, DMArray *monsters, const char *fileName)
{
    printf("11\n");
    ...
    // Load player's data from the first line
    fscanf(file, "%d\n", &(player->x) + 1);
    // fscanf(file, "%d %d %d\n", &(player->x) + 1, &(player->y) + 1, &(player->HP));
    printf("44\n");
    ...
    // Load monsters' data
    // while (monsters->len < 10)
    // {
    //     printf("55\n");
    //     struct Monster tempMonster;
    //     if (fscanf(file, "%s %d %d %d %d %d\n", tempMonster.name.string, &(tempMonster.x) + 1, &(tempMonster.y) + 1,\
    //                                              &(tempMonster.HP), &(tempMonster.attack), &(tempMonster.isAlive)) != EOF)
    //     {
    //         printf("66\n");
    //         struct Monster *monster = (struct Monster *)malloc(sizeof(struct Monster));
    //         *monster = tempMonster;
    //         InsertArray(monsters, monster);
    //     }
    //     else
    //     {
    //         break;
